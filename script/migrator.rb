...
def self.create_record(visit_encounter_id, encounter)
  case encounter.name.upcase
    ...
    when 'HEIGHT/WEIGHT'
	      ...
	      current_height = Observation.find(:last, :conditions => ["concept_id = ? and patient_id = ?",Concept.find_by_name("Height").concept_id,encounter.patient_id]) rescue nil
		      ...
		      case ob.concept.name.upcase
		        when 'PREGNANT'
		        	enc.patient_pregnant = Concept.find(ob.value_coded).name
		  			when 'BREASTFEEDING'
		  				enc.patient_breast_feeding = pregnant.Concept(ob.value_coded).name
        ...
        when 'PREGNANT'
        	enc.patient_pregnant = Concept.find(ob.value_coded).name
  			when 'BREASTFEEDING'
				...
				#when 'OTHER SYMPTOMS'
					#enc.other_symptoms = Concept(ob.value_coded).name
					...
					if enc.diarrhoea.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_diarrhoea = Concept(ob.value_coded).name
					end
					if enc.jaundice.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_jaundice = Concept(ob.value_coded).name
					end
					if enc.leg_pain_numbness.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_leg_pain_numbness = Concept(ob.value_coded).name
					end
					...
					if enc.peripheral_neuropathy.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_peripheral_neuropathy = Concept(ob.value_coded).name
					end
					if enc.hepatitis.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_hepatitis = Concept(ob.value_coded).name
					end
					if enc.anaemia.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_anaemia = Concept(ob.value_coded).name
					end
					if enc.lactic_acidosis.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_lactic_acidosis = Concept(ob.value_coded).name
					end
					if enc.lipodystrophy.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_lipodystrophy = Concept(ob.value_coded).name
					end
					if enc.skin_rash.to_upcase == 'YES DRUG INDUCED' 
						enc.drug_induced_skin_rash = Concept(ob.value_coded).name
					end
